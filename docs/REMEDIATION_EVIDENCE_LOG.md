...
## CHANGE-060 — Atomic consumable reservation fulfillment (2026-09-26)

- CHANGE-059 verification run 36243897113 completed successfully. New tracked migration `backend/src/migrations/20260926_reservation_fulfillment.sql` adds fulfilled reservation state, fulfillment actor/time and FK to physical stock movement.
- New online `POST /projects/:id/reservations/:reservationId/fulfill` requires `inventory.adjust_stock` AND project editor access. It locks the reservation and item rows in one PostgreSQL transaction, rejects non-confirmed reservations and returnable equipment/tools/instruments, consumes the full reserved quantity, ensures other confirmed reservations remain physically covered, writes the inventory movement, updates physical stock and marks the reservation fulfilled with the movement ID. Repeated fulfillment requests return the original movement and do not deduct again. The existing audit helper is invoked after commit; audit logging failure is logged and does not undo the committed physical transaction.
- Desktop API and project reservation panel expose fulfillment with an explicit in-app confirmation, avoiding unreliable native `window.confirm`. Reservation status typing includes fulfilled. All commits are on main; CI for CHANGE-060 is pending at documentation time.

LIMITATIONS: This supports FULL-quantity fulfillment of non-returnable reservations only; no partial fulfillment or equipment checkout/return. Confirmed equipment reservations remain blocked from this endpoint. Migration execution and concurrent PostgreSQL tests are not yet verified. Other stock writers, direct DB writes and offline conflict reconciliation require a comprehensive audit. Physical quantity and reservation status are atomic in the new endpoint, but post-commit application audit log is best effort, not transactional. UI currently presents the fulfillment action on confirmed reservations generally; the backend rejects returnable item types. Do not treat this as a complete inventory enforcement system.


## CHANGE-061 — Partial consumable reservation fulfillment (2026-09-26)

- CHANGE-060 run 36260335139 passed all CI jobs. Added tracked `20260926_partial_reservation_fulfillment.sql`: immutable initial `original_quantity`, cumulative `fulfilled_quantity`, and per-movement `project_reservation_fulfillments` history. Existing `quantity` now represents REMAINING allocation for confirmed reservations after partial fulfillment.
- Updated online fulfillment endpoint to require an explicit positive quantity with up to three decimal places, reject fulfillment exceeding remaining allocation, serialize by reservation and item locks, ensure all other confirmed allocations remain covered, atomically write consumable stock movement, decrement remaining quantity, increment cumulative fulfilled quantity, and mark reservation fulfilled only when fully consumed. A separate GET endpoint lists fulfillment movements. Returnable equipment remains excluded.
