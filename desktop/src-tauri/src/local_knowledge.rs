...
#[tauri::command]
pub fn update_local_knowledge_finding(app:AppHandle,id:String,patch:Value)->Result<Value,String>{update_entity(app,"findings","finding",id,patch)}
#[tauri::command]
pub fn delete_local_knowledge_finding(app:AppHandle,id:String)->Result<(),String>{delete_entity(app,"findings","finding",id)}
#[tauri::command]
...
#[tauri::command]
pub fn update_local_knowledge_result(app:AppHandle,id:String,patch:Value)->Result<Value,String>{update_entity(app,"results","knowledge_result",id,patch)}
#[tauri::command]
pub fn delete_local_knowledge_result(app:AppHandle,id:String)->Result<(),String>{delete_entity(app,"results","knowledge_result",id)}
...
#[tauri::command]
pub fn delete_local_knowledge_relationship(app:AppHandle,id:String)->Result<(),String>{delete_entity(app,"relationships","knowledge_relationship",id)}
#[tauri::command]
pub fn search_local_knowledge(app:AppHandle,q:String)->Result<Value,String>{let c=conn(&app)?;let s=load(&c)?;let findings=query(arr(&s,"findings"),&q);let results=query(arr(&s,"results"),&q);let notes:Vec<Value>=serde_json::from_str(&c.query_row("SELECT value FROM sync_state WHERE key='notes_state'",[],|r|r.get(0)).optional().map_err(|e|e.to_string())?.unwrap_or_else(||"[]".into())).unwrap_or_else(|_|Vec::new());Ok(json!({"findings":findings,"results":results,"notes":query(notes,&q)}))}
#[tauri::command]
pub fn get_local_knowledge_overview(app:AppHandle)->Result<Value,String>{let c=conn(&app)?;let s=load(&c)?;let findings=arr(&s,"findings");let results=arr(&s,"results");let notes_state:Value=serde_json::from_str(&c.query_row("SELECT value FROM sync_state WHERE key='notes_state'",[],|r|r.get(0)).optional().map_err(|e|e.to_string())?.unwrap_or_else(||"[]".into())).unwrap_or(json!([]));let notes=notes_state.as_array().cloned().unwrap_or_default();let resources_state:Value=serde_json::from_str(&c.query_row("SELECT value FROM sync_state WHERE key='resources_state'",[],|r|r.get(0)).optional().map_err(|e|e.to_string())?.unwrap_or_else(||"[]".into())).unwrap_or(json!([]));let resources=resources_state.as_array().cloned().unwrap_or_default();Ok(json!({"counts":{"notes":notes.len(),"resources":resources.len()},"categories":[],"recent_notes":notes.iter().rev().take(5).cloned().collect::<Vec<_>>(),"recent_resources":resources.iter().rev().take(5).cloned().collect::<Vec<_>>(),"findings":findings.len(),"results":results.len()}))}
#[tauri::command]
