...
#[tauri::command]
pub fn update_local_engineering_test(app:AppHandle,id:String,patch:Value)->Result<Value,String>{let mut c=conn(&app)?;let mut rows=load(&c,TEST_KEY)?;let r=rows.iter_mut().find(|r|r.get("id").and_then(Value::as_str)==Some(id.as_str())).ok_or("Engineering test not found")?;if let Some(obj)=patch.as_object(){for (k,v) in obj {if !["id","created_at"].contains(&k.as_str()){r[k]=v.clone();}}}if r.get("title").and_then(Value::as_str).map(|s|s.trim().is_empty()).unwrap_or(true){return Err("title is required".into())}r["updated_at"]=json!(now(&c)?);let updated=r.clone();save(&mut c,TEST_KEY,&rows,Some(("engineering_test",&id,"update",&updated)))?;Ok(updated)}
#[tauri::command]
...
pub fn apply_server_engineering_pull(app:AppHandle,calculations:Vec<Value>,tests:Vec<Value>,deleted_calculation_ids:Vec<String>,deleted_test_ids:Vec<String>)->Result<(),String>{
 ...
 let pending:std::collections::HashSet<String>={let mut s=std::collections::HashSet::new();let mut st=c.prepare("SELECT entity_type,entity_id FROM sync_outbox WHERE synced_at IS NULL AND entity_type IN ('engineering_calculation','engineering_test')").map_err(|e|e.to_string())?;let it=st.query_map([],|r|Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?))).map_err(|e|e.to_string())?;for x in it.flatten(){s.insert(format!("{}:{}",x.0,x.1.unwrap_or_default()));}s};
