...
fn mutate(app:&AppHandle,key:&str,entity:&str,op:&str,mut value:Value)->Result<Value,String>{let mut conn=open_local_connection(app)?;let permission=if op=="delete"{"finance.delete"}else if op=="create"&&entity=="transaction"&&value.get("direction").and_then(Value::as_str)==Some("income"){"finance.create_income"}else if op=="create"&&entity=="transaction"{"finance.create_expense"}else{"finance.edit"};
