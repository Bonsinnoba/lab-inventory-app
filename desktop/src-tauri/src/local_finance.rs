...
fn write(conn:&rusqlite::Connection,key:&str,v:&Vec<Value>)->Result<(),String>{conn.execute("INSERT INTO sync_state(key,value) VALUES(?1,?2) ON CONFLICT(key) DO UPDATE SET value=excluded.value",[key,serde_json::to_string(v).map_err(|e|e.to_string())?]).map_err(|e|e.to_string())?;Ok(())}
...
fn mutate(app:&AppHandle,key:&str,entity:&str,op:&str,mut value:Value)->Result<Value,String>{let mut conn=open_local_connection(app)?;let mut rows=read(&conn,key)?;let entity_id=value.get("id").and_then(Value::as_str).ok_or("id is required")?.to_string();if op=="delete"{rows.retain(|v|v.get("id").and_then(Value::as_str)!=Some(&entity_id));}else if let Some(old)=rows.iter_mut().find(|v|v.get("id").and_then(Value::as_str)==Some(&entity_id)){*old=value.clone();}else{rows.push(value.clone());}write(&conn,key,&rows)?;queue(&mut conn,entity,&entity_id,op,&value)?;Ok(value)}
fn all(app:&AppHandle,key:&str)->Result<Vec<Value>,String>{let c=open_local_connection(app)?;read(&c,key)}
fn merge(app:&AppHandle,key:&str,entity:&str,incoming:Vec<Value>,deleted:Vec<String>)->Result<(),String>{let mut c=open_local_connection(app)?;let mut rows=read(&c,key)?;let pending:std::collections::HashSet<String>={let mut s=c.prepare("SELECT entity_id FROM sync_outbox WHERE synced_at IS NULL AND entity_type=?1 AND entity_id IS NOT NULL").map_err(|e|e.to_string())?;s.query_map([entity],|r|r.get(0)).map_err(|e|e.to_string())?.filter_map(Result::ok).collect()};rows.retain(|v|!deleted.contains(&v.get("id").and_then(Value::as_str).unwrap_or(""))||pending.contains(v.get("id").and_then(Value::as_str).unwrap_or("")));for x in incoming{let xid=x.get("id").and_then(Value::as_str).unwrap_or("");if pending.contains(xid){continue}if let Some(old)=rows.iter_mut().find(|v|v.get("id").and_then(Value::as_str)==Some(xid)){*old=x}else{rows.push(x)}}write(&c,key,&rows)}
