...
import { Download, Filter, RotateCcw, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1400px] mx-auto w-full">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-6">
        ...
        <button onClick={() => exportLedgerToCsv(rowsWithBalance)} disabled={!transactions || transactions.length === 0} className="w-full sm:w-auto flex items-center justify-center gap-2 px-3 py-2 bg-surface-raised border border-border rounded-sm hover:border-accent transition-colors text-sm disabled:opacity-40 disabled:cursor-not-allowed focus:outline-none focus:ring-1 focus:ring-accent"><Download size={16} /> Export CSV</button>
      </div>
      <div className="bg-surface border border-border rounded-md p-3 mb-4">
        <div className="flex flex-col xl:flex-row xl:items-center gap-3">
          <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-text-secondary shrink-0"><Filter size={15} /> Filters</div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full xl:w-auto"><label className="sr-only" htmlFor="ledger-from">From</label><input id="ledger-from" type="date" aria-label="From date" value={dateRange.from} onChange={(e) => setDateRange({ ...dateRange, from: e.target.value })} className="w-full sm:w-auto bg-bg border border-border rounded-sm px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent" /><span className="hidden sm:inline text-text-secondary">to</span><label className="sr-only" htmlFor="ledger-to">To</label><input id="ledger-to" type="date" aria-label="To date" value={dateRange.to} onChange={(e) => setDateRange({ ...dateRange, to: e.target.value })} className="w-full sm:w-auto bg-bg border border-border rounded-sm px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent" /></div>
          <label className="sr-only" htmlFor="ledger-direction">Transaction direction</label><select id="ledger-direction" value={directionFilter} onChange={(e) => setDirectionFilter(e.target.value)} className="w-full xl:w-auto bg-bg border border-border rounded-sm px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent"><option value="">All directions</option><option value="income">Income only</option><option value="expense">Expense only</option></select>
          <label className="sr-only" htmlFor="ledger-budget">Budget period</label><select id="ledger-budget" value={budgetPeriodFilter} onChange={(e) => setBudgetPeriodFilter(e.target.value)} className="w-full xl:w-auto bg-bg border border-border rounded-sm px-3 py-2 text-text-primary text-sm focus:outline-none focus:border-accent"><option value="">All budget periods</option>{budgetPeriods?.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}</select>
          {hasFilters && <button onClick={resetFilters} className="flex items-center justify-center gap-1.5 px-3 py-2 text-sm text-text-secondary hover:text-text-primary transition-colors focus:outline-none focus:ring-1 focus:ring-accent rounded-sm"><RotateCcw size={14} /> Reset</button>}
