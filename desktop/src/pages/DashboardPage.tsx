...
export default function DashboardPage(){
 ...
 return <div className="page-frame space-y-5">
  ...
  <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
   ...
   <section className="workspace-card ui-panel rounded-md p-5" aria-labelledby="activity-heading"><h2 id="activity-heading" className="text-section-header font-semibold mb-4">Recent lab activity</h2>{!recent.length?<p className="text-sm text-text-secondary">No recent activity.</p>:<><div className="space-y-2">{recent.slice(0,8).map((x:any)=>{ const destination=x.type==='note'?'/notebook':x.type==='resource'?'/resources':`/projects/${x.project_id}`; const icon=x.type==='experiment'?<FlaskConical size={15}/>:x.type==='task'?<ClipboardList size={15}/>:<Activity size={15}/>; return <Link key={`${x.type}-${x.id}`} to={destination} className="flex items-center gap-3 py-2 px-1 rounded hover:text-accent hover:bg-surface-raised transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/70"><span className="text-accent shrink-0">{icon}</span><span className="flex-1 min-w-0"><strong className="block text-sm truncate">{x.title}</strong><span className="text-[10px] text-text-secondary">{x.project_name||x.type}</span></span><span className="text-[10px] text-text-secondary whitespace-nowrap">{new Date(x.created_at).toLocaleDateString()}</span></Link>; })}</div><ShowingLimit shown={Math.min(8,recent.length)} total={recent.length} label="activities"/></>}</section>
  </div>
 </div>
}
