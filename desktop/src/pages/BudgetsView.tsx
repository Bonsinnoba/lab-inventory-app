...
import { Plus, Trash2, Edit, Wallet, Layers, Landmark, X, Check, RotateCcw } from 'lucide-react';
...
export default function BudgetsView() {
  ...
  const saveProjectBudget = (id: string) => {
    ...
    if (isNaN(value) || value < 0) { showToast('Enter a valid budget amount', 'error'); return; }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1400px] mx-auto w-full space-y-8">
      <div>
        <p className="text-[11px] uppercase tracking-[0.16em] text-text-secondary mb-1">Financial setup</p>
        <h1 className="text-section-header font-ui font-semibold">Budgets</h1>
        <p className="text-text-secondary text-sm mt-1 max-w-2xl">What's <em>allocated</em> — separate from the Ledger, which tracks what's actually happened.</p>
      </div>

      <section>
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
          <div>
            <h2 className="text-text-primary font-medium flex items-center gap-2"><Wallet size={18} className="text-accent" /> Lab Budget</h2>
            <p className="text-text-secondary text-xs mt-1 max-w-2xl">Whole-lab allocations for a fiscal year, quarter, or other defined period.</p>
          </div>
          <button onClick={() => setShowBudgetModal(true)} className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-2 px-3 py-2 bg-accent text-bg rounded-sm hover:bg-accent-dim transition-colors text-sm focus:outline-none focus:ring-1 focus:ring-accent"><Plus size={14} /> Add Lab Budget</button>
        </div>
        {loadingPeriods ? <div className="text-text-secondary text-sm py-8 text-center bg-surface border border-border rounded-md">Loading lab budgets…</div> : periodsError ? (
          <div className="text-center py-8 bg-surface border border-border rounded-md"><p className="text-status-danger text-sm">Unable to load lab budgets.</p><button onClick={() => retry(refetchPeriods)} className="mt-3 px-3 py-1.5 border border-border rounded-sm text-sm hover:border-accent">Try again</button></div>
        ) : !budgetPeriods?.length ? (
          <div className="text-text-secondary text-sm py-8 text-center bg-surface border border-border rounded-md"><p>No lab budget set up yet.</p><p className="text-xs mt-1">Add a period to start tracking your lab-wide allocation.</p></div>
        ) : (
          <div className="bg-surface border border-border rounded-md overflow-hidden">
            <div className="px-4 py-3 border-b border-border text-xs text-text-secondary">{budgetPeriods.length} budget period{budgetPeriods.length === 1 ? '' : 's'}</div>
            <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-sm"><thead className="bg-surface-raised border-b border-border"><tr><th className="text-left px-4 py-3 text-text-secondary font-medium">Label</th><th className="text-left px-4 py-3 text-text-secondary font-medium">Period</th><th className="text-right px-4 py-3 text-text-secondary font-medium font-mono">Total</th><th className="w-16 px-4 py-3"></th></tr></thead><tbody>
              {budgetPeriods.map((period) => <tr key={period.id} className="border-b border-border last:border-0 hover:bg-surface-raised transition-colors"><td className="px-4 py-3 text-text-primary font-medium">{period.label}</td><td className="px-4 py-3 text-text-secondary whitespace-nowrap">{period.start_date ? new Date(period.start_date).toLocaleDateString() : '—'} – {period.end_date ? new Date(period.end_date).toLocaleDateString() : '—'}</td><td className="px-4 py-3 text-right font-mono text-text-primary">{money(period.total_budget)}</td><td className="px-4 py-3 text-right"><button aria-label={`Delete ${period.label}`} title="Delete budget period" onClick={() => deleteBudgetPeriodMutation.mutate(period.id)} disabled={deleteBudgetPeriodMutation.isPending} className="p-1.5 text-text-secondary hover:text-status-danger disabled:opacity-40 rounded-sm focus:outline-none focus:ring-1 focus:ring-accent"><Trash2 size={14} /></button></td></tr>)}
            </tbody></table></div>
          </div>
        )}
      </section>
