...
export async function cacheLocalInventorySnapshot(items: Item[]): Promise<void> {
  try { await invoke('cache_local_inventory_snapshot', { snapshotJson: JSON.stringify(items) }); } catch {}
}

async function getLocalState<T>(key: string, fallback: T): Promise<T> {
  try {
    const raw = await invoke<string | null>('get_local_sync_state', { key });
...
async function saveSnapshotWithSync(
  ...
  await invoke('save_local_snapshot_with_sync', {
    input: {
      ...
      stateJson: stateValue === undefined ? null : JSON.stringify(stateValue),
    },
  });
}

export async function createLocalInventoryItem(item: Omit<Item, 'id' | 'created_at' | 'updated_at'>): Promise<Item | null> {
  const local = await getLocalInventorySnapshot(); if (!local) return null;
  const now = new Date().toISOString();
  const created: Item = { ...item, id: crypto.randomUUID(), created_at: now, updated_at: now };
  await saveSnapshotWithSync([...local, created], { entity_type: 'item', entity_id: created.id, operation: 'create', payload: created });
  return created;
}

export async function updateLocalInventoryItem(id: string, patch: Partial<Item>): Promise<Item | null> {
  const local = await getLocalInventorySnapshot(); if (!local) return null;
  const index = local.findIndex((item) => item.id === id); if (index < 0) throw new Error('Item not found in local inventory');
  const updated: Item = { ...local[index], ...patch, id, updated_at: new Date().toISOString() };
  const next = [...local]; next[index] = updated;
  await saveSnapshotWithSync(next, { entity_type: 'item', entity_id: id, operation: 'update', payload: { id, patch, item: updated } });
  return updated;
}

export async function deleteLocalInventoryItem(id: string): Promise<boolean> {
  const local = await getLocalInventorySnapshot(); if (!local) return false;
  if (!local.some((item) => item.id === id)) throw new Error('Item not found in local inventory');
  await saveSnapshotWithSync(local.filter((item) => item.id !== id), { entity_type: 'item', entity_id: id, operation: 'delete', payload: { id } });
  return true;
}

export async function bulkUpdateLocalInventoryStatus(ids: string[], status: Item['status']): Promise<number | null> {
  const local = await getLocalInventorySnapshot(); if (!local) return null;
  const idSet = new Set(ids); let updatedCount = 0;
  const next = local.map((item) => { if (!idSet.has(item.id)) return item; updatedCount += 1; return { ...item, status, updated_at: new Date().toISOString() }; });
  if (updatedCount === 0) return 0;
  await saveSnapshotWithSync(next, { entity_type: 'item', operation: 'bulk_status', payload: { ids, status } });
  return updatedCount;
}

export async function bulkDeleteLocalInventoryItems(ids: string[]): Promise<number | null> {
  const local = await getLocalInventorySnapshot(); if (!local) return null;
  const idSet = new Set(ids); const deleted = local.filter((item) => idSet.has(item.id)).map((item) => item.id);
  if (deleted.length === 0) return 0;
  await saveSnapshotWithSync(local.filter((item) => !idSet.has(item.id)), { entity_type: 'item', operation: 'bulk_delete', payload: { ids: deleted } });
  return deleted.length;
}

export async function getLocalItemMovements(id: string): Promise<ItemMovement[] | null> {
  const local = await getLocalInventorySnapshot(); if (!local) return null;
  return getLocalState<ItemMovement[]>(`inventory_movements:${id}`, []);
}

export async function createLocalItemMovement(
  id: string,
  movement: { movement_type: MovementType; quantity: number; to_storage_location?: string; to_location_id?: string; project_id?: string; reason?: string; reference?: string },
): Promise<{ item: Item; movement: ItemMovement } | null> {
  const local = await getLocalInventorySnapshot(); if (!local) return null;
  const index = local.findIndex((item) => item.id === id); if (index < 0) throw new Error('Item not found in local inventory');
  const quantity = Number(movement.quantity);
  if (!Number.isFinite(quantity) || quantity <= 0) throw new Error('Quantity must be greater than zero');
  const allowed: MovementType[] = ['receive','checkout','return','consume','adjust','transfer','damage','loss','repair_out','repair_in'];
  ...
  if (movement.movement_type === 'transfer' && !movement.to_storage_location?.trim() && !movement.to_location_id) throw new Error('A destination location is required for transfers');

  const now = new Date().toISOString();
  const movementRecord: ItemMovement = {
    id: crypto.randomUUID(), item_id: id, movement_type: movement.movement_type, quantity,
    ...
    to_storage_location: movement.movement_type === 'transfer' ? (movement.to_storage_location?.trim() || null) : (local[index].storage_location ?? null),
    project_id: movement.project_id ?? null, reason: movement.reason ?? null, reference: movement.reference ?? null,
    created_at: now,
  };
  const updated: Item = {
    ...local[index], current_quantity: after,
  ...
  const next = [...local]; next[index] = updated;
  const history = await getLocalItemMovements(id) ?? [];
  await saveSnapshotWithSync(next, {
    entity_type: 'item_movement', entity_id: id, operation: 'create',
    payload: { movement: movementRecord, item: updated },
  }, `inventory_movements:${id}`, [...history, movementRecord]);
  return { item: updated, movement: movementRecord };
}

export async function getLocalInventoryOrRemote(remoteLoader: () => Promise<Item[]>, filters?: { type?: string; status?: string; location?: string; location_id?: string; low_stock?: boolean }): Promise<Item[]> {
  const local = await getLocalInventorySnapshot();
  if (local !== null) return applyInventoryFilters(local, filters);
  const remote = await remoteLoader(); await cacheLocalInventorySnapshot(remote); return applyInventoryFilters(remote, filters);
}
...
function applyInventoryFilters(items: Item[], filters?: { type?: string; status?: string; location?: string; location_id?: string; low_stock?: boolean }): Item[] {
  if (!filters) return items; const query = filters.location?.trim().toLowerCase();
