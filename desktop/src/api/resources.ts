...
async function enrichWithDownloadedMedia(resources: Resource[]): Promise<Resource[]> {
  try {
    ...
    return resources.map(resource => {
      ...
      return {
        ...resource,
        local_media_filename: job.local_media_filename,
        local_media_size_bytes: job.local_media_size_bytes,
        local_media_downloaded_at: job.completed_at || job.updated_at || new Date().toISOString(),
      };
    });
  } catch {
    return resources;
...
export async function getResource(id: string): Promise<Resource> { const response = await apiFetch(`/resources/${id}`); if (!response.ok) throw new Error('Failed to fetch resource'); return (await enrichWithDownloadedMedia([await response.json()]))[0]; }
export async function getAllResources(): Promise<Resource[]> { const response = await apiFetch(`/resources`); if (!response.ok) throw new Error('Failed to fetch resources'); return enrichWithDownloadedMedia(await response.json()); }
export async function getResources(filters: { item_id?: string; project_id?: string; note_id?: string; parent_resource_id?: string }): Promise<Resource[]> {
  ...
  const response = await apiFetch(`/resources?${params}`); if (!response.ok) throw new Error('Failed to fetch resources'); return enrichWithDownloadedMedia(await response.json());
}
...
export async function createFolder(name: string, parent: { item_id?: string; project_id?: string; note_id?: string }): Promise<Resource> { const response = await apiFetch(`/resources/folder`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, ...parent }) }); if (!response.ok) throw new Error(await getApiErrorMessage(response, 'Failed to create folder')); return response.json(); }
...
export async function createLink(url: string, parent: { item_id?: string; project_id?: string; note_id?: string }, name?: string, metadata?: { category?: string; description?: string; tags?: string[] }): Promise<Resource> { const response = await apiFetch(`/resources/link`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url, name, ...parent, ...metadata }) }); if (!response.ok) throw new Error(await getApiErrorMessage(response, 'Failed to create link')); return response.json(); }
export async function updateResourceMetadata(id: string, metadata: { category?: string; description?: string; tags?: string[] }): Promise<Resource> { const response = await apiFetch(`/resources/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(metadata) }); if (!response.ok) throw new Error(await getApiErrorMessage(response, 'Failed to update resource metadata')); return response.json(); }
export async function deleteResource(id: string): Promise<void> { const response = await apiFetch(`/resources/${id}`, { method: 'DELETE' }); if (!response.ok) throw new Error(await getApiErrorMessage(response, 'Failed to delete resource')); }
