...
export interface Phase4Item {
  id: string; name: string; type: string; status: string;
  current_quantity: number; initial_quantity: number; unit?: string | null;
  storage_location?: string | null; storage_container_id?: string | null;
  storage_container_name?: string | null; location_name?: string | null;
}
...
async function json<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(body?.error?.message || body?.error || 'Phase 4 request failed');
  return body;
}
