...
export interface ItemMovement {
  id: string; item_id: string; movement_type: MovementType; quantity: number; quantity_before: number; quantity_after: number;
  from_location_id?: string | null; to_location_id?: string | null; from_location_name?: string | null; to_location_name?: string | null;
  from_storage_location?: string | null; to_storage_location?: string | null;
  project_id?: string | null; project_name?: string | null; reason?: string | null; reference?: string | null;
  performed_by?: string | null; performed_by_username?: string | null; created_at: string;
}
...
export async function getItemMovements(id: string): Promise<ItemMovement[]> { const response = await apiFetch(`/items/${id}/movements`); if (!response.ok) throw new Error('Failed to fetch item movements'); return response.json(); }
export async function createItemMovement(id: string, movement: { movement_type: MovementType; quantity: number; to_storage_location?: string; to_location_id?: string; project_id?: string; reason?: string; reference?: string; }): Promise<{ item: Item; movement: ItemMovement }> { const response = await apiFetch(`/items/${id}/movements`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(movement) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body?.error?.message || body?.error || 'Failed to record movement'); return body; }
