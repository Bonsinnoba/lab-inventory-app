...
export interface DownloadJob {
  ...
  bytes_downloaded?: number; total_bytes?: number; error_message?: string; attempts: number;
  started_at?: string; completed_at?: string; created_at: string;
}
export interface DownloadSettings { enabled: boolean; mode: 'manual'|'scheduled'|'always'; window_start: string; window_end: string; concurrent_downloads: number; default_quality: string; }
...
export async function queueVideoDownload(resource_id:string, quality?:string, scheduled_for?:string):Promise<DownloadJob>{ const r=await apiFetch('/media-downloads/queue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({resource_id,quality,scheduled_for})}); if(!r.ok) throw new Error(await getApiErrorMessage(r,'Failed to queue video download')); return r.json(); }
export async function updateDownloadJob(id:string,status:DownloadJob['status']):Promise<DownloadJob>{ const r=await apiFetch(`/media-downloads/queue/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})}); if(!r.ok) throw new Error(await getApiErrorMessage(r,'Failed to update download job')); return r.json(); }
