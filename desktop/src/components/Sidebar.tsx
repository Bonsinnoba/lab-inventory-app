  ...
  return <>
    ...
    <aside style={{ width: collapsed ? COLLAPSED_WIDTH : width }} aria-label="LabOS navigation" className={`desktop-sidebar relative bg-surface border-r border-border flex flex-col flex-shrink-0 ${collapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      {user && <div className="relative p-3 border-t border-border sidebar-user-area">
        {collapsed ? <>
          <button type="button" onClick={() => setShowCollapsedProfile(value => !value)} aria-expanded={showCollapsedProfile} aria-haspopup="menu" aria-label={`${user.display_name || user.username} profile`} title={`${user.display_name || user.username} profile`} className={`mx-auto w-10 h-10 bg-accent rounded-full flex items-center justify-center transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/70 ${showCollapsedProfile ? 'ring-2 ring-accent/50' : ''}`}><User size={18} className="text-bg" aria-hidden="true" /></button>
          {showCollapsedProfile && <div role="menu" aria-label="Profile menu" className="absolute left-[calc(100%+10px)] bottom-2 z-50 w-72 rounded-xl border border-border bg-surface-raised shadow-2xl p-2.5">
