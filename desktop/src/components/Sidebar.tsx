import { Box, ChevronDown, DollarSign, FileBarChart, Layers, LogOut, PanelLeftClose, PanelLeftOpen, ScanLine, Sun, User, Users, BookOpen, Settings, LayoutDashboard, Download } from 'lucide-react';
  ...
  return <>
    ...
    <aside style={{ width: collapsed ? COLLAPSED_WIDTH : width }} aria-label="LabOS navigation" className={`desktop-sidebar relative bg-surface border-r border-border flex flex-col flex-shrink-0 ${collapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      {user && <div className="relative p-3 border-t border-border sidebar-user-area">
        {collapsed ? <>
          <button type="button" onClick={() => setShowCollapsedProfile(value => !value)} aria-expanded={showCollapsedProfile} aria-haspopup="menu" aria-label={`${user.display_name || user.username} profile`} title={`${user.display_name || user.username} profile`} className="mx-auto w-9 h-9 bg-accent rounded-full flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/70"><User size={17} className="text-bg" aria-hidden="true" /></button>
          {showCollapsedProfile && <div role="menu" aria-label="Profile actions" className="absolute left-[calc(100%+8px)] bottom-2 z-30 w-52 rounded-lg border border-border bg-surface shadow-xl p-2">
            <div className="px-2 py-2 border-b border-border mb-1"><p className="text-sm font-medium text-text-primary truncate">{user.display_name || user.username}</p><p className="text-xs text-text-secondary capitalize">{user.role}</p></div>
            <button type="button" role="menuitem" onClick={() => setMode(theme.mode === 'light' ? 'dark' : 'light')} className="w-full flex items-center gap-3 px-2.5 py-2 rounded-md text-sm text-text-secondary hover:bg-surface-raised hover:text-text-primary"><Sun size={16} aria-hidden="true" />{theme.mode === 'light' ? 'Dark mode' : 'Light mode'}</button>
            <Link to="/settings" role="menuitem" onClick={() => setShowCollapsedProfile(false)} className="w-full flex items-center gap-3 px-2.5 py-2 rounded-md text-sm text-text-secondary hover:bg-surface-raised hover:text-text-primary"><Settings size={16} aria-hidden="true" />Settings</Link>
            {onLogout && <button type="button" role="menuitem" onClick={onLogout} className="w-full flex items-center gap-3 px-2.5 py-2 rounded-md text-sm text-text-secondary hover:bg-surface-raised hover:text-text-primary"><LogOut size={16} aria-hidden="true" />Sign out</button>}
          </div>}
        </> : <>
