...
export default function ProjectReservations({project,canEdit}:{project:Project;canEdit:boolean}){
 ...
 const fulfill=useMutation({mutationFn:({id,quantity}:{id:string;quantity:number})=>fulfillProjectReservation(project.id,id,quantity),onSuccess:()=>{setError('');qc.invalidateQueries({queryKey:['project-reservations',project.id]});qc.invalidateQueries({queryKey:['items']});},onError:(e:Error)=>setError(e.message)});
 ...
 return <section className="border border-border rounded-md bg-surface p-4 my-4" aria-label="Project reservations">
  ...
  {fulfillTarget&&<div role="alertdialog" aria-modal="true" aria-label="Confirm reservation fulfillment" className="border border-status-warning rounded-sm p-3 mt-3"><p className="text-sm">Consume the specified confirmed quantity and permanently record the stock movement? This action cannot be undone.</p><label className="text-xs block mt-2">Quantity to consume (maximum {reservations.data?.find(r=>r.id===fulfillTarget)?.quantity})<input type="number" min="0.001" step="0.001" max={Number(reservations.data?.find(r=>r.id===fulfillTarget)?.quantity||0)} value={fulfillQuantity} onChange={e=>setFulfillQuantity(e.target.value)} className="block w-full bg-bg border border-border rounded-sm p-2 mt-1"/></label><div className="flex gap-2 mt-2"><button type="button" className="ui-button ui-button-primary ui-button-sm" disabled={fulfill.isPending||!(Number(fulfillQuantity)>0)||Number(fulfillQuantity)>Number(reservations.data?.find(r=>r.id===fulfillTarget)?.quantity||0)} onClick={()=>fulfill.mutate({id:fulfillTarget,quantity:Number(fulfillQuantity)},{onSuccess:()=>setFulfillTarget(null)})}>Confirm consumption</button><button type="button" className="ui-button ui-button-sm" onClick={()=>setFulfillTarget(null)}>Cancel</button></div></div>}
  ...
  <ul className="mt-3 space-y-2">{reservations.data?.map(r=><li key={r.id} className="border border-border rounded-sm p-3 text-sm">
   ...
   {isAdmin&&r.status==='confirmed'&&<button type="button" disabled={fulfill.isPending} className="ui-button ui-button-primary ui-button-sm mt-2 mr-2" onClick={()=>{setFulfillTarget(r.id);setFulfillQuantity(String(r.quantity));}}>Fulfill consumables</button>}
