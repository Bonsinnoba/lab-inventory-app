...
export default function ProjectBomPanel({ projectId, canEdit }: { projectId: string; canEdit: boolean }) {
  ...
  return (
    <section className="bg-surface border border-border rounded-md overflow-hidden">
      ...
      {pendingDelete && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4"><div role="alertdialog" aria-modal="true" aria-labelledby="bom-delete-title" className="w-full max-w-md rounded-lg border border-border bg-surface p-5 shadow-xl" onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();setPendingDelete(null)}if(e.key==='Tab'){const buttons=Array.from(e.currentTarget.querySelectorAll('button')) as HTMLButtonElement[];const active=buttons.indexOf(document.activeElement as HTMLButtonElement);if(e.shiftKey&&active===0){e.preventDefault();buttons[buttons.length-1]?.focus()}else if(!e.shiftKey&&active===buttons.length-1){e.preventDefault();buttons[0]?.focus()}}}}><h3 id="bom-delete-title" className="text-lg font-semibold">Remove BOM component?</h3><p className="my-4 text-sm">Permanently remove "{pendingDelete.name}" from this project's bill of materials? This cannot be undone.</p><div className="flex justify-end gap-2"><button autoFocus type="button" onClick={()=>setPendingDelete(null)} className="rounded border border-border px-4 py-2">Cancel</button><button type="button" disabled={remove.isPending} onClick={()=>{const id=pendingDelete.id;setPendingDelete(null);remove.mutate(id)}} className="rounded bg-status-danger px-4 py-2 text-white disabled:opacity-50">Remove permanently</button></div></div></div>}
    </section>
  );
}
