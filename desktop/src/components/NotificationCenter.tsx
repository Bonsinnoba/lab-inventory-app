...
export default function NotificationCenter(){
 ...
 useEffect(()=>{if(!open)return;const onKeyDown=(e:KeyboardEvent)=>{if(e.key==='Escape'){e.preventDefault();setOpen(false);}};window.addEventListener('keydown',onKeyDown);return()=>window.removeEventListener('keydown',onKeyDown)},[open]);
 ...
 return <div className="relative">
   <button type="button" onClick={()=>setOpen(v=>!v)} aria-label={`Notifications${unread?`, ${unread} unread`:''}`} aria-expanded={open} aria-haspopup="dialog" title="Notifications" className="relative p-2 hover:bg-surface-raised rounded-sm text-text-secondary hover:text-text-primary">
