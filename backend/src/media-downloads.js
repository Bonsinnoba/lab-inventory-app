...
function run(command, args, { jobId, onProgress } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.stderr.on('data', c => {
      const text = c.toString(); stderr += text;
...
async function getStopState(jobId) {
  ...
  if (!r.rowCount) return 'cancelled';
  const row = r.rows[0];
  return row.stop_requested_status || (row.cancel_requested ? 'cancelled' : null);
}
async function cleanMediaFiles(dir, base) {
  const entries = await fs.readdir(dir).catch(() => []);
  await Promise.all(entries.filter(n => n.startsWith(`${base}.`) && !/^thumbnail\./i.test(n)).map(n => fs.rm(path.join(dir, n), { force: true })));
}
async function startJob(job) {
  ...
  try {
    await run(YTDLP, args, { jobId: job.id, onProgress: async p => {
      ...
      if (stop) { terminate(processes.get(job.id)); return; }
    ...
    if (stop) {
      ...
      await pool.query(`UPDATE resource_download_jobs SET status=$1,cancel_requested=$2,stop_requested_status=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=$3`, [stop, stop === 'cancelled', job.id]);
      return;
    }
    const entries = await fs.readdir(dir);
    const mediaName = entries.find(n => n.startsWith(`${base}.`) && !/^thumbnail\./i.test(n));
    ...
    const stop = await getStopState(job.id).catch(() => null);
    await cleanMediaFiles(dir, base);
    if (stop) {
      await pool.query(`UPDATE resource_download_jobs SET status=$1,cancel_requested=$2,stop_requested_status=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=$3`, [stop, stop === 'cancelled', job.id]);
      return;
    }
    const current = (await pool.query('SELECT attempts,max_attempts FROM resource_download_jobs WHERE id=$1', [job.id])).rows[0];
...
async function recoverInterruptedJobs() {
  ...
  await pool.query(`UPDATE resource_download_jobs SET status=COALESCE(stop_requested_status,'cancelled'),stop_requested_status=NULL,updated_at=CURRENT_TIMESTAMP WHERE status='downloading' AND cancel_requested=TRUE`);
}
async function tick() {
  if (ticking) return;
  ticking = true;
  try {
    const settings = (await pool.query('SELECT * FROM media_download_settings WHERE id=1')).rows[0];
    if (!settings?.enabled) return;
    const now = new Date();
    const withinWindow = isWithinWindow(settings.window_start, settings.window_end, now);
    if (settings.mode === 'scheduled' && !withinWindow) {
      const dueScheduledJobs = await pool.query(`SELECT id FROM resource_download_jobs WHERE status IN ('queued','scheduled') AND cancel_requested=FALSE AND scheduled_for IS NOT NULL AND scheduled_for <= CURRENT_TIMESTAMP LIMIT 1`);
      if (!dueScheduledJobs.rowCount) return;
    }
    ...
    if (!available) return;
    let eligibility;
    if (settings.mode === 'manual') eligibility = '(scheduled_for IS NOT NULL AND scheduled_for <= CURRENT_TIMESTAMP)';
    else if (settings.mode === 'scheduled' && !withinWindow) eligibility = '(scheduled_for IS NOT NULL AND scheduled_for <= CURRENT_TIMESTAMP)';
    else eligibility = '(scheduled_for IS NULL OR scheduled_for <= CURRENT_TIMESTAMP)';
...
export async function stopMediaDownloadJob(jobId, finalStatus = 'cancelled') {
  const status = ['paused','cancelled'].includes(finalStatus) ? finalStatus : 'cancelled';
  await pool.query(`UPDATE resource_download_jobs SET cancel_requested=TRUE,stop_requested_status=$1,status=CASE WHEN status IN ('queued','scheduled','paused','failed') THEN $1 ELSE status END,updated_at=CURRENT_TIMESTAMP WHERE id=$2`, [status, jobId]);
