...
const checks = [
  ...
  ['worker downloads mandatory YouTube thumbnail before video', worker.includes('ensureYouTubeThumbnail(resource)') && worker.includes('before the video worker starts downloading')],
  ['thumbnail download uses yt-dlp', worker.includes("'--skip-download', '--write-thumbnail'")],
  ...
  ['API exposes media tool diagnostics', api.includes('getMediaToolsStatus') && api.includes('/media-downloads/tools/status')],
