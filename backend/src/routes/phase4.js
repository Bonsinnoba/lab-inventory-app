...
router.get('/items', async (req, res) => {
  ...
  if (search) {
    values.push(`%${search}%`);
    where.push(`(i.name ILIKE $${values.length} OR COALESCE(i.sku,'') ILIKE $${values.length} OR COALESCE(i.storage_location,'') ILIKE $${values.length} OR COALESCE(sc.name,'') ILIKE $${values.length} OR COALESCE(sc.storage_location,'') ILIKE $${values.length})`);
  }
  const predicate = where.length ? `WHERE ${where.join(' AND ')}` : '';
  try {
    const count = await pool.query(`SELECT COUNT(*)::int AS total FROM items i LEFT JOIN storage_containers sc ON sc.id = i.storage_container_id ${predicate}`, values);
    values.push(pageSize, offset);
    const result = await pool.query(`SELECT i.*, l.name AS location_name, sc.name AS storage_container_name, sc.storage_location AS storage_container_location FROM items i LEFT JOIN locations l ON l.id = i.location_id LEFT JOIN storage_containers sc ON sc.id = i.storage_container_id ${predicate} ORDER BY i.name ASC LIMIT $${values.length - 1} OFFSET $${values.length}`, values);
    res.json({ items: result.rows, ...pageMeta(page, pageSize, count.rows[0].total) });
  } catch (err) { console.error(err); res.status(500).json({ error: { code: 'PHASE4_ITEMS_FAILED', message: 'Failed to fetch paginated inventory' } }); }
});

router.get('/containers', async (req, res) => {
  const page = Math.max(1, Number(req.query.page) || 1); const pageSize = clampPageSize(req.query.page_size); const offset = (page - 1) * pageSize;
  try {
    const count = await pool.query('SELECT COUNT(*)::int AS total FROM storage_containers');
    const result = await pool.query(`SELECT sc.*, COUNT(i.id)::int AS item_count FROM storage_containers sc LEFT JOIN items i ON i.storage_container_id = sc.id GROUP BY sc.id ORDER BY sc.name ASC LIMIT $1 OFFSET $2`, [pageSize, offset]);
...
router.patch('/items/:id/storage', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query('UPDATE items SET storage_location = $1, storage_container_id = $2 WHERE id = $3 RETURNING *', [storage_location?.trim() || null, storage_container_id || null, req.params.id]);
    ...
    res.json(result.rows[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: { code: 'ITEM_STORAGE_UPDATE_FAILED', message: 'Failed to update item storage' } }); }
});
...
router.get('/intelligence', async (_req, res) => {
  try {
    ...
    res.json({ inventory: i, maintenance: m, storage: s, signals: [...(Number(i.depleted) > 0 ? [{ level: 'critical', message: `${i.depleted} item(s) are depleted` }] : []), ...(Number(i.low_stock) > 0 ? [{ level: 'warning', message: `${i.low_stock} item(s) are at or below the 20% stock threshold` }] : []), ...(Number(m.due) > 0 ? [{ level: 'attention', message: `${m.due} maintenance task(s) are due within 14 days` }] : []), ...(Number(s.total) > Number(s.labeled) ? [{ level: 'attention', message: `${Number(s.total) - Number(s.labeled)} item(s) have no free-form storage label` }] : [])] });
  } catch (err) { console.error(err); res.status(500).json({ error: { code: 'INTELLIGENCE_FAILED', message: 'Failed to calculate scientific inventory signals' } }); }
});
