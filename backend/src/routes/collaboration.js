...
router.get('/activity', hasPermission('projects.view'), async (req, res) => {
  ...
  try {
    ...
    console.error(err); res.status(500).json({ error: 'Failed to fetch activity' });
  }
});
...
router.get('/notifications', async (req, res) => {
  ...
  try {
    const result = await pool.query(`SELECT id,type,title,body,entity_type,entity_id,metadata,read_at,created_at
      FROM notifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT $2`, [req.user.userId, limit]);
...
async function canAccessProject(projectId, userId, role) {
  ...
  const result = await pool.query(`SELECT 1 FROM projects p LEFT JOIN project_members pm ON pm.project_id=p.id AND pm.user_id=$2
    WHERE p.id=$1 AND (p.owner_id=$2 OR pm.user_id=$2)`, [projectId, userId]);
...
router.get('/projects/:projectId/comments', hasPermission('projects.view'), async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT c.id,c.project_id,c.author_id,c.body,c.created_at,c.updated_at,u.username AS author_username
      FROM project_comments c LEFT JOIN users u ON u.id=c.author_id WHERE c.project_id=$1 ORDER BY c.created_at ASC`, [req.params.projectId]);
...
router.post('/projects/:projectId/comments', hasPermission('projects.edit'), async (req, res) => {
  ...
  try {
    if (!(await canAccessProject(req.params.projectId, req.user.userId, req.user.role))) return res.status(403).json({ error: 'Project access required' });
    const result = await pool.query(`INSERT INTO project_comments(project_id,author_id,body) VALUES($1,$2,$3)
      RETURNING id,project_id,author_id,body,created_at,updated_at`, [req.params.projectId, req.user.userId, body]);
    ...
    for (const member of members.rows) {
      await pool.query(`INSERT INTO notifications(user_id,type,title,body,entity_type,entity_id,metadata) VALUES($1,'project_comment','New project comment',$2,'project_comment',$3,$4)`,
        [member.user_id, `${req.user.username} commented on a project.`, result.rows[0].id, JSON.stringify({project_id:req.params.projectId})]);
    }
