...
router.get('/projects/pull',async(req,res,next)=>{
  try{
    ...
    const taskMap=by(tasks.rows,'project_id'),taskExperimentMap=by(taskExperiments.rows,'task_id'),taskExperimentProjectMap=by(taskExperiments.rows,'project_id'),experimentMap=by(experiments.rows,'project_id'),bomMap=by(bom.rows,'project_id'),itemMap=by(items.rows,'project_id'),blockMap=by(blocks.rows,'project_id'),connectorMap=by(connectors.rows,'project_id'),measurementMap=by(measurements.rows,'experiment_id'),observationMap=by(observations.rows,'experiment_id'),attachmentTaskMap=by(attachments.rows,'task_id'),attachmentExperimentMap=by(attachments.rows,'experiment_id'),requirementMap=by(requirements.rows,'project_id'),taskExperimentProjectMap=by(taskExperiments.rows,'project_id');
