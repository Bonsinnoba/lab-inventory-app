...
async function applyProjectEntity(client,change,user){const entityType=change.entity_type,cfg=PROJECT_ENTITY_CONFIG[entityType],payload=object(change.payload,'Project sync payload');const record=payload.project||payload.item||payload.record||payload;const entityId=id(record.id||payload.id||change.entity_id,'Project entity ID');let projectId=entityType==='project'?entityId:record.project_id;if(!projectId&&['project_experiment_measurement','project_experiment_observation'].includes(entityType)&&record.experiment_id){const parent=await client.query('SELECT project_id FROM project_experiments WHERE id=$1',[record.experiment_id]);projectId=parent.rows[0]?.project_id||null;}if(!projectId&&entityType==='project_work_attachment'){const work=record.task_id||record.experiment_id;if(work){const parent=await client.query('SELECT project_id FROM project_tasks WHERE id=$1 UNION ALL SELECT project_id FROM project_experiments WHERE id=$1 LIMIT 1',[work]);projectId=parent.rows[0]?.project_id||null;}}if(!projectId){const existing=await client.query('SELECT project_id FROM '+cfg.table+' WHERE id=$1',[entityId]);projectId=existing.rows[0]?.project_id||null;}if(entityType!=='project'&&!(await canEditProject(client,projectId,user)))fail(403,'PROJECT_ACCESS_DENIED','You do not have edit access to this project');if(entityType==='project'&&change.operation!=='create'&&!(await canEditProject(client,entityId,user)))fail(403,'PROJECT_ACCESS_DENIED','You do not have edit access to this project');validateProjectRecord(entityType,record);if(change.operation==='create'){const existing=await client.query('SELECT * FROM '+cfg.table+' WHERE id=$1',[entityId]);if(existing.rowCount)return existing.rows[0];if(entityType!=='project'&&!projectId)fail(400,'INVALID_PROJECT_REFERENCE','project_id is required');const values=[entityId],columns=['id'];for(const field of cfg.fields){if(field==='owner_id'&&entityType==='project'&&!record.owner_id){columns.push(field);values.push(user.userId);continue;}if(Object.prototype.hasOwnProperty.call(record,field)){columns.push(field);values.push(record[field]??null);}}if(entityType==='project_connector'){const checks=await client.query('SELECT id,project_id FROM project_blocks WHERE id=ANY($1::uuid[])',[[record.source_block_id,record.target_block_id]]);if(checks.rowCount!==2||checks.rows.some(row=>row.project_id!==projectId))fail(400,'INVALID_PROJECT_CONNECTOR','Connector blocks must belong to this project');}const placeholders=values.map((_,i)=>'$'+(i+1)).join(',');return(await client.query('INSERT INTO '+cfg.table+' ('+columns.join(',')+') VALUES ('+placeholders+') RETURNING *',values)).rows[0];}if(change.operation==='update'){const existing=await client.query('SELECT * FROM '+cfg.table+' WHERE id=$1 FOR UPDATE',[entityId]);if(!existing.rowCount)fail(409,'PROJECT_ENTITY_NOT_FOUND',entityType+' '+entityId+' does not exist on the server');const updates=[],values=[];for(const field of cfg.fields){if(['project_id','created_by','owner_id'].includes(field))continue;if(Object.prototype.hasOwnProperty.call(record,field)){values.push(record[field]??null);updates.push(field+'=$'+values.length);}}if(!updates.length)return existing.rows[0];updates.push('updated_at=now()');values.push(entityId);return(await client.query('UPDATE '+cfg.table+' SET '+updates.join(',')+' WHERE id=$'+values.length+' RETURNING *',values)).rows[0];}if(change.operation==='delete'){const result=await client.query('DELETE FROM '+cfg.table+' WHERE id=$1 RETURNING id',[entityId]);if(result.rowCount)await client.query("INSERT INTO sync_tombstones(entity_type,entity_id) VALUES($1,$2) ON CONFLICT(entity_type,entity_id) DO UPDATE SET deleted_at=now()",[entityType,entityId]);return{deleted:Boolean(result.rowCount),id:entityId};}fail(400,'UNSUPPORTED_PROJECT_OPERATION','Unsupported project operation: '+change.operation);}
...
const RESOURCE_FIELDS=['name','kind','file_type','original_filename','mime_type','size_bytes','parent_resource_id','relative_path','url','thumbnail_url','item_id','project_id','note_id','category','description','tags'];async function applyResourceEntity(client,change,user){const payload=object(change.payload,'Resource sync payload'),record=payload.resource||payload.record||payload,entityId=id(record.id||payload.id||change.entity_id,'Resource ID');if(change.operation==='create'){const permissions=await getUserPermissions(user.userId,user.role);const needed=change.operation==='create'?'resources.create':change.operation==='delete'?'resources.delete':'resources.edit';if(!permissions.has(needed))fail(403,'PERMISSION_DENIED',`Permission required: ${needed}`);if(!['link','folder'].includes(String(record.kind||'')))fail(400,'UNSUPPORTED_RESOURCE_CREATE','Only link and folder resources can be created through offline sync');if(!String(record.name||'').trim())fail(400,'INVALID_RESOURCE','Resource name is required');const parents=[record.item_id,record.project_id,record.note_id].filter(Boolean);if(parents.length>1)fail(400,'INVALID_RESOURCE_PARENT','A resource can be attached to at most one parent context');if(record.project_id){const access=await client.query("SELECT 1 FROM projects p WHERE p.id=$1 AND (p.owner_id=$2 OR $3='admin' OR EXISTS (SELECT 1 FROM project_members pm WHERE pm.project_id=p.id AND pm.user_id=$2))",[record.project_id,user.userId,user.role]);if(!access.rowCount)fail(403,'PROJECT_ACCESS_DENIED','You do not have access to this project');}const existing=await client.query('SELECT * FROM resources WHERE id=$1',[entityId]);if(existing.rowCount)return existing.rows[0];const values=[entityId],columns=['id'];for(const field of RESOURCE_FIELDS){if(field==='file_type'&&record.kind==='folder'&&!record.file_type){columns.push(field);values.push('schematic_folder');continue;}if(Object.prototype.hasOwnProperty.call(record,field)){columns.push(field);values.push(record[field]??null);}}columns.push('uploaded_by');values.push(user.userId);return(await client.query(`INSERT INTO resources (${columns.join(',')}) VALUES (${values.map((_,i)=>i+1).join(',')}) RETURNING *`,values)).rows[0];}const existing=await client.query('SELECT * FROM resources WHERE id=$1 FOR UPDATE',[entityId]);if(!existing.rowCount)fail(409,'RESOURCE_NOT_FOUND',`Resource ${entityId} does not exist on the server`);const access=await getResourceAccess(entityId,{userId:user.userId,role:user.role});if(access.access!=='edit')fail(403,'RESOURCE_ACCESS_DENIED','You do not have edit access to this resource');if(change.operation==='update'){const updates=[],values=[];for(const field of ['category','description','tags'])if(Object.prototype.hasOwnProperty.call(record,field)){values.push(record[field]??null);updates.push(field+'=$'+values.length);}if(!updates.length)return existing.rows[0];values.push(entityId);return(await client.query('UPDATE resources SET '+updates.join(',')+' WHERE id=$'+values.length+' RETURNING *',values)).rows[0];}if(change.operation==='delete'){await client.query('DELETE FROM resources WHERE id=$1',[entityId]);await client.query("INSERT INTO sync_tombstones(entity_type,entity_id) VALUES('resource',$1) ON CONFLICT(entity_type,entity_id) DO UPDATE SET deleted_at=now()",[entityId]);return{id:entityId,deleted:true};}fail(400,'UNSUPPORTED_RESOURCE_OPERATION','Unsupported resource operation: '+change.operation);}
const FINANCE_CONFIG={transaction:{table:'transactions',permission:{create:'finance.create_expense',update:'finance.edit',delete:'finance.delete'}},budget_period:{table:'budget_periods',permission:{create:'finance.edit',update:'finance.edit',delete:'finance.delete'}},funding_source:{table:'funding_sources',permission:{create:'finance.edit',update:'finance.edit',delete:'finance.delete'}}};async function applyFinanceEntity(client,change,user){const cfg=FINANCE_CONFIG[change.entity_type],required=cfg?.permission?.[change.operation];if(required){const permissions=await getUserPermissions(user.userId,user.role);if(!permissions.has(required))fail(403,'PERMISSION_DENIED',`Permission required: ${required}`);}const payload=object(change.payload,'Finance sync payload'),record=payload.transaction||payload.budget_period||payload.funding_source||payload.record||payload,entityId=id(record.id||payload.id||change.entity_id,'Finance ID');const existing=await client.query(`SELECT * FROM ${cfg.table} WHERE id=$1`,[entityId]);if(change.operation==='create'){if(existing.rowCount)return existing.rows[0];}else if(!existing.rowCount)fail(409,'FINANCE_NOT_FOUND',`Finance record ${entityId} does not exist`);if(change.operation==='delete'){await client.query(`DELETE FROM ${cfg.table} WHERE id=$1`,[entityId]);await client.query("INSERT INTO sync_tombstones(entity_type,entity_id) VALUES($1,$2) ON CONFLICT(entity_type,entity_id) DO UPDATE SET deleted_at=now()",[change.entity_type,entityId]);return{id:entityId,deleted:true};}const allowed=cfg.table==='transactions'?['type','direction','amount','date','vendor','notes','item_id','project_id','funding_source_id','budget_period_id']:cfg.table==='budget_periods'?['label','total_budget','start_date','end_date','notes']:['name','source_type','contact_info','notes'];const fields=allowed.filter(k=>Object.prototype.hasOwnProperty.call(record,k));if(change.operation==='create'&&cfg.table==='transactions'&&!fields.includes('direction'))fail(400,'INVALID_TRANSACTION','direction is required');if(change.operation==='create'){const cols=['id',...fields],vals=[entityId,...fields.map(k=>record[k]??null)];return(await client.query(`INSERT INTO ${cfg.table} (${cols.join(',')}) VALUES (${vals.map((_,i)=>i+1).join(',')}) RETURNING *`,vals)).rows[0];}if(!fields.length)return existing.rows[0];const sets=fields.map((field,i)=>field+'=$'+(i+1)),vals=fields.map(k=>record[k]??null);vals.push(entityId);return(await client.query(`UPDATE ${cfg.table} SET ${sets.join(',')} WHERE id=$${vals.length} RETURNING *`,vals)).rows[0];}
...
router.get('/resources/pull',async(req,res,next)=>{
  try{
    ...
    const deleted=await pool.query("SELECT entity_id FROM sync_tombstones WHERE entity_type='resource' ORDER BY deleted_at DESC LIMIT 500");
...
router.get('/engineering/pull',async(req,res,next)=>{
 try{
  ...
  const [calculations,tests,tomb]=await Promise.all([
   ...
   pool.query("SELECT entity_type,entity_id FROM sync_tombstones WHERE entity_type IN ('engineering_calculation','engineering_test') ORDER BY deleted_at DESC LIMIT 1000")
  ]);
