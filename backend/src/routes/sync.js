...
const ITEM_FIELDS=['name','type','category','sku','initial_quantity','current_quantity','unit','dimensions','status','condition_notes','unit_cost','replacement_cost','location_id','storage_location','photo_url','supplier','supplier_id','part_number','acquisition_method','acquisition_source','acquisition_notes','next_maintenance_date','maintenance_interval_days','manufacturer','model_number','serial_number','asset_tag','calibration_interval_days','next_calibration_date','assigned_to','image_resource_id','created_at','updated_at'];
...
async function applyProjectEntity(client,change,user){
  ...
  if(entityType==='project'&&change.operation==='update'&&existing?.status==='planning'&&record.status==='active')fail(409,'PROJECT_REVIEW_REQUIRED','Submit the plan for online admin review before activation');
