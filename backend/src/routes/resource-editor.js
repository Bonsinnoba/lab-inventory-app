...
function extension(resource) { return path.extname(resource.original_filename || resource.name || '').toLowerCase(); }
function isMarkdownResource(resource) { const name = (resource.original_filename || resource.name || '').toLowerCase(); return resource.file_type === 'text' && (name.endsWith('.md') || name.endsWith('.markdown')); }
function isDocxResource(resource) { return resource.file_type === 'document' && extension(resource) === '.docx'; }
function isPdfResource(resource) { return resource.file_type === 'pdf' && extension(resource) === '.pdf'; }
...
router.get('/:id/content', async (req, res) => {
  try {
    ...
    if (!isMarkdownResource(found.resource)) return res.status(400).json({ error: 'Only Markdown resources can be edited as text' });
    const content = await fs.readFile(resolveStoragePath(found.resource.storage_path), 'utf8');
    res.type('text/plain').send(content);
  } catch (err) { console.error(err); res.status(500).json({ error: err.message || 'Failed to read resource content' }); }
});
...
router.put('/:id/content', async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: err.message || 'Failed to save Markdown content' }); }
});
...
router.get('/:id/docx', async (req, res) => {
  try {
    const found = await getEditableResource(req.params.id, req.user);
    if (!found.resource) return res.status(found.status).json({ error: found.error });
    if (!isDocxResource(found.resource)) return res.status(400).json({ error: 'Only DOCX resources are supported' });
    const buffer = await fs.readFile(resolveStoragePath(found.resource.storage_path));
    const result = await mammoth.convertToHtml({ buffer }, { styleMap: [
      "p[style-name='Title'] => h1", "p[style-name='Heading 1'] => h2", "p[style-name='Heading 2'] => h3", "p[style-name='Heading 3'] => h4",
    ] });
    res.json({ html: result.value, messages: result.messages || [] });
  } catch (err) { console.error(err); res.status(500).json({ error: err.message || 'Failed to render DOCX resource' }); }
});
...
router.post('/:id/pdf-copy', async (req, res) => {
  try {
    ...
    res.status(201).json(created);
  } catch (err) { console.error(err); res.status(500).json({ error: err.message || 'Failed to create editable PDF copy' }); }
});

// Compatibility endpoint. The Resources UI should use pdf-copy so originals stay immutable.
router.put('/:id/file', upload.single('file'), async (req, res) => {
  try {
    ...
    const result = await pool.query('UPDATE resources SET original_filename = $1, mime_type = $2, size_bytes = $3, file_type = $4, updated_at = CURRENT_TIMESTAMP WHERE id = $5 RETURNING *', [req.file.originalname, req.file.mimetype, req.file.size, fileType, resource.id]);
    ...
    res.json(result.rows[0]);
  } catch (err) { console.error(err); res.status(500).json({ error: err.message || 'Failed to replace PDF resource' }); }
});
