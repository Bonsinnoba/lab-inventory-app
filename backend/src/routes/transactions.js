...
router.get('/', hasPermission('finance.view'), async (req, res) => {
  ...
  if (budget_period_id) { values.push(budget_period_id); conditions.push(`t.budget_period_id = $${values.length}`); }
...
router.get('/summary', hasPermission('finance.view'), async (req, res) => {
  ...
  try {
    ...
    const byMonth = await pool.query(`SELECT to_char(date_trunc('month', date), 'YYYY-MM') AS month, direction, type, SUM(amount)::float AS total FROM transactions ${where} GROUP BY 1, direction, type ORDER BY 1 ASC`, values);
    response.by_month = byMonth.rows;
    if (budget_period_id) {
      ...
      if (budgetPeriod.rows.length > 0) {
        ...
        const spentResult = await pool.query(`SELECT COALESCE(SUM(amount), 0)::float AS total FROM transactions WHERE budget_period_id = $1 AND direction = 'expense'`, [budget_period_id]);
        const spent = Number(spentResult.rows[0].total || 0); const totalBudget = Number(period.total_budget || 0);
...
router.post('/', async (req, res, next) => {
  ...
  if (funding_source_id) { const sourceCheck = await pool.query('SELECT id FROM funding_sources WHERE id = $1', [funding_source_id]); if (sourceCheck.rows.length === 0) return res.status(400).json({ error: 'funding_source_id references a non-existent funding source' }); }
  ...
  if (budget_period_id) {
    ...
    const txDate = date || new Date().toISOString().slice(0, 10); const { start_date, end_date } = period.rows[0];
...
router.put('/:id', hasPermission('finance.edit'), async (req, res) => {
  const fields = ['type', 'direction', 'amount', 'date', 'vendor', 'notes', 'item_id', 'project_id', 'funding_source_id', 'budget_period_id']; const updates = []; const values = [];
  ...
  if ('type' in req.body || 'direction' in req.body) {
    const current = await pool.query('SELECT type, direction, funding_source_id FROM transactions WHERE id = $1', [req.params.id]); if (!current.rows.length) return res.status(404).json({ error: 'Transaction not found' });
    const currentType = req.body.type ?? current.rows[0].type; const currentDirection = req.body.direction ?? current.rows[0].direction; const currentFundingSourceId = req.body.funding_source_id !== undefined ? req.body.funding_source_id : current.rows[0].funding_source_id;
  ...
  if ('funding_source_id' in req.body && req.body.funding_source_id) { const sourceCheck = await pool.query('SELECT id FROM funding_sources WHERE id = $1', [req.body.funding_source_id]); if (!sourceCheck.rows.length) return res.status(400).json({ error: 'funding_source_id references a non-existent funding source' }); }
  const current = await pool.query('SELECT * FROM transactions WHERE id=$1', [req.params.id]); if (!current.rows.length) return res.status(404).json({ error: 'Transaction not found' });
  const effectiveDirection = req.body.direction ?? current.rows[0].direction; const effectiveType = req.body.type ?? current.rows[0].type; const effectiveProjectId = req.body.project_id ?? current.rows[0].project_id;
...
router.delete('/:id', hasPermission('finance.delete'), requireRole('admin'), async (req, res) => {
  try { const result = await pool.query('DELETE FROM transactions WHERE id = $1 RETURNING *', [req.params.id]); if (!result.rows.length) return res.status(404).json({ error: 'Transaction not found' }); await writeAuditLog({ req, action: 'DELETE', entityType: 'transaction', entityId: req.params.id, oldValue: result.rows[0] }); res.status(204).send(); }
