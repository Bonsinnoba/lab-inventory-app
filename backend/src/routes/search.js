...
router.get('/', async (req, res) => {
  ...
  try {
    const rows = await Promise.all(types.map(async (type) => { const result = await pool.query(TYPE_QUERIES[type], [q, req.user.userId, req.user.role]); return [type, result.rows.map((row) => decorate(type, row))]; }));
    ...
    res.setHeader('Cache-Control', 'no-store'); res.json({ query: q, counts, total: all.length, all, ...grouped });
