...
router.get('/:id/movements', async (req, res) => { try { const result = await pool.query(`SELECT m.*,p.name AS project_name,fl.name AS from_location_name,tl.name AS to_location_name,u.username AS performed_by_username FROM item_movements m LEFT JOIN projects p ON p.id=m.project_id LEFT JOIN locations fl ON fl.id=m.from_location_id LEFT JOIN locations tl ON tl.id=m.to_location_id LEFT JOIN users u ON u.id=m.performed_by WHERE m.item_id=$1 ORDER BY m.created_at DESC`, [req.params.id]); res.json(result.rows); } catch (err) { console.error(err); res.status(500).json({ error: { code: 'MOVEMENTS_FETCH_FAILED', message: 'Failed to fetch item movements' } }); } });
router.post('/:id/movements', hasPermission('inventory.adjust_stock'), async (req, res) => {
  const { movement_type, quantity, to_location_id = null, project_id = null, reason = null, reference = null } = req.body; const allowed = new Set(['receive','checkout','return','consume','adjust','transfer','damage','loss','repair_out','repair_in']); const qty = Number(quantity);
  if (!allowed.has(movement_type)) return res.status(400).json({ error: { code: 'INVALID_MOVEMENT_TYPE', message: 'Invalid movement type' } }); if (!Number.isFinite(qty) || qty <= 0) return res.status(400).json({ error: { code: 'INVALID_QUANTITY', message: 'Quantity must be greater than zero' } }); if (movement_type === 'transfer' && !to_location_id) return res.status(400).json({ error: { code: 'TRANSFER_LOCATION_REQUIRED', message: 'A destination location is required for transfers' } });
  const client = await pool.connect();
  try {
    await client.query('BEGIN'); const itemResult = await client.query('SELECT * FROM items WHERE id=$1 FOR UPDATE', [req.params.id]); if (!itemResult.rowCount) { await client.query('ROLLBACK'); return res.status(404).json({ error: { code: 'ITEM_NOT_FOUND', message: 'Item not found' } }); }
    ...
    let locationId = item.location_id; if (movement_type === 'transfer') locationId = to_location_id;
    const movement = await client.query(`INSERT INTO item_movements (item_id,movement_type,quantity,from_location_id,to_location_id,project_id,reason,reference,performed_by) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`, [req.params.id,movement_type,qty,item.location_id,locationId,project_id,reason,reference,req.user?.userId || null]);
    const updated = await client.query('UPDATE items SET current_quantity=$1,location_id=$2 WHERE id=$3 RETURNING *', [next,locationId,req.params.id]); await client.query('COMMIT'); await writeAuditLog({ req, action: 'CREATE', entityType: 'item_movement', entityId: movement.rows[0].id, newValue: movement.rows[0] }); res.status(201).json({ movement: movement.rows[0], item: updated.rows[0] });
