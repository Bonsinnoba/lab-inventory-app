...
['local media uses IndexedDB',()=>{const m=read('../desktop/src/components/MusicPlayer.tsx');return m.includes('indexedDB.open(DB, VERSION)')&&m.includes("const DB = 'labos-local-media'")&&m.includes("TRACK_STORE = 'tracks'")}],
['music never uploads to LabOS',()=>{const m=read('../desktop/src/components/MusicPlayer.tsx');return (m.includes('never uploaded to LabOS')||m.includes('never sent to LabOS'))&&m.includes('labos-local-media')}],
...
['music topbar launcher removed',()=>!read('../desktop/src/components/TopBar.tsx').includes('labos:music-player')],
