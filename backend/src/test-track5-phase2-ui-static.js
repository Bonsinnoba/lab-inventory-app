...
check(/right-0 bottom-\[66px\].*pointer-events-auto/, 'hover widget remains interactable');
check(/ref=\{minimizedWidgetRef\}/, 'hover widget has a stable interaction ref');
check(/onMouseEnter=\{\(\) => \{\s*if \(minimizedHoverTimer\.current\) clearTimeout\(minimizedHoverTimer\.current\);\s*setMinimizedHover\(true\);/s, 'hover trigger is the minimized icon');
if (/ref=\{minimizedWidgetRef\}\s*\n\s*onMouseEnter/.test(s)) { console.error('FAIL: hover widget does not trigger hover state'); failures++; } else console.log('✓ hover widget does not trigger hover state');
check(/setTimeout\(\(\) => setMinimizedHover\(false\), 600\)/, 'icon leave uses a travel grace period');
